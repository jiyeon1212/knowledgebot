...
- intent가 "search"이면 search_keyword에 핵심 검색 키워드를 추출, entities는 null
  - **중요**: 고유명사(회사명, 인물명, 프로젝트명 등)는 반드시 search_keyword에 포함하세요
  - 예: "미래에셋 사업 제안 관련 자료 찾아줘" → search_keyword: "미래에셋 사업 제안"
  - 예: "삼성전자 계약서 검색해줘" → search_keyword: "삼성전자 계약서"
  - "관련", "자료", "찾아줘", "검색해줘" 같은 동작어는 제외하되, 고유명사와 핵심 명사는 반드시 포함
...
{"intent": "search 또는 chat 또는 entity_search", "search_keyword": "키워드 또는 null", "max_results": 숫자 또는 null, "entities": [{"name": "엔티티명", "type": "고객사|담당자|프로젝트|팀"}] 또는 null}
...
async def classify_intent(user_text: str) -> dict:
    ...
    한 번의 Gemini 호출로 의도 분류(search/chat/entity_search) + 키워드 추출
    + 결과 수 추출 + 엔티티 추출을 동시에 수행한다.
    ...
    """
    client = _get_client()
    prompt = INTENT_CLASSIFICATION_PROMPT + user_text
...
async def generate_chat_response(user_text: str) -> str:
    """일반 대화 의도에 대해 Gemini로 직접 대화 응답을 생성한다."""
    client = _get_client()
